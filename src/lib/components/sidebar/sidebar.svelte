...
{#if sharedState.space}
  ...
  <div class="flex size-full flex-col justify-between">
    ...
    <div class="group/explorer flex w-full grow items-start justify-center overflow-y-auto">
      {#if isCollapsed}
        <TooltipProvider>
          <Tooltip delayDuration={500} disableHoverableContent>
            <TooltipTrigger
              class={cn(buttonVariants({ variant: "ghost", size: "icon" }), "my-1.5 flex-shrink-0")}
              onclick={() => {
                pane.expand();
                pane.resize(24);
              }}
      ...
      {:else}
        <div class="size-full">
          <p class="text-muted-foreground flex h-[36px] items-center px-[22px]">Explorer</p>
          <TreeNodeRoot root={spaceSnapshot.root_collection}>
            {#if explorerActionsState.createNewNode === "request" && explorerState.isCreateNewNodeParent(rootRelpath)}
              <TreeNodeCreate type="request" locationRelpath={rootRelpath} level={1} />
            {/if}
            ...
            {#if explorerActionsState.createNewNode === "collection" && explorerState.isCreateNewNodeParent(rootRelpath)}
              <TreeNodeCreate type="collection" locationRelpath={rootRelpath} level={1} />
            {/if}
