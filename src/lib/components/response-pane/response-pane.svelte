  ...
  >
    ...
    {:else}
      {#if openReq.self.status === "Success" || openReq.self.status === "Error"}
        <div class="px-1.5">
          <TabsList class="grid auto-cols-min grid-flow-col justify-start gap-2 p-0 [&>*]:text-xs">
            <TabsTrigger value="body">Body</TabsTrigger>
            <TabsTrigger value="cookies">
              {"Cookies".concat(
                openReq.self.response?.cookies ? ` (${openReq.self.response?.cookies.length})` : "",
              )}
            </TabsTrigger>
            <TabsTrigger value="headers">
              {"Headers".concat(
                openReq.self.response?.headers ? ` (${openReq.self.response?.headers.length})` : "",
              )}
            </TabsTrigger>
          </TabsList>
        </div>
  ...
  {#if !isCollapsed}
    <div class="bg-background flex h-[calc(100%-32px)] w-full">
      ...
      <TabsContent value="cookies" class="m-0 size-full">
        ...
        {:else}
          <div class="flex size-full items-center justify-center">No cookies for you :(</div>
        {/if}
      </TabsContent>
      <TabsContent value="headers" class="m-0 size-full">
        ...
        {:else}
          <div class="flex size-full items-center justify-center">No headers received</div>
        {/if}
      </TabsContent>
    </div>
  {/if}
