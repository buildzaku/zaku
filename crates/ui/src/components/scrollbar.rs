use gpui::{
    Along, Anchor, AnyElement, App, Axis as ScrollbarAxis, BorderStyle, Bounds, ContentMask,
    Corners, CursorStyle, DispatchPhase, Div, Edges, ElementId, Entity, EntityId, Global,
    GlobalElementId, Hitbox, HitboxBehavior, Hsla, InspectorElementId, IsZero, LayoutId, ListState,
    MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Point, Position,
    ScrollHandle, ScrollWheelEvent, Size, Stateful, Style, Task, UniformListDecoration,
...
fn get_scrollbar_state<T>(
    ...
    caller_location: &'static panic::Location,
    window: &mut Window,
    cx: &mut App,
    ...
    fn vertical_scrollbar_for<ScrollHandle: ScrollableHandle + Clone>(
        ...
        self.custom_scrollbars(
            Scrollbars::new(ScrollAxes::Vertical)
                ...
                .ensure_id(panic::Location::caller()),
            window,
            cx,
        )
    }
...
impl WithScrollbar for Stateful<Div> {
    ...
    where
        T: ScrollableHandle,
    {
        let scrollbar = get_scrollbar_state(config, panic::Location::caller(), window, cx);
...
impl Scrollbars {
    fn new_with_setting(show_along: ScrollAxes, get_visibility: fn(&App) -> ShowScrollbar) -> Self {
        Self {
            ...
            notify_target: None,
            visibility: show_along.apply_to(&Point::default(), ReservedSpace::Thumb),
            track_color: None,
...
impl<ScrollHandle: ScrollableHandle> Scrollbars<ScrollHandle> {
    ...
    pub fn notify_content(mut self) -> Self {
        self.notify_target = Some(NotifyTarget::Parent);
        self
    }

    pub fn tracked_entity(mut self, entity_id: EntityId) -> Self {
        self.notify_target = Some(NotifyTarget::Entity(entity_id));
        self
    }
...
pub fn on_new_scrollbars<T: Global>(cx: &mut App) {
...
impl<T: ScrollableHandle> ScrollbarState<T> {
    fn new_from_config(config: Scrollbars<T>, parent_id: EntityId, cx: &mut Context<Self>) -> Self {
        ...
        let notify_id = config.notify_target.map(|target| match target {
            NotifyTarget::Parent => parent_id,
            NotifyTarget::Entity(entity_id) => entity_id,
        });
    ...
    fn schedule_auto_hide(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if self.auto_hide_task.is_none() {
            self.auto_hide_task = (self.visible() && self.show_behavior == ShowBehavior::Autohide)
                .then(|| {
    ...
    fn compute_click_offset(
        ...
        let thumb_start =
            (event_position.along(axis) - track_bounds.origin.along(axis) - thumb_offset)
                .clamp(gpui::px(0.0), viewport_size - thumb_size);
    ...
    fn prepaint(
        ...
        prepaint_state.map(|state| {
            let autohide_delta = self
                ...
                .show_state
                .animation_progress()
                .and_then(|(delta, delta_duration, should_invert)| {
                    id.map(|id| {
                        window.with_element_state(id, |state, window| {
                            ...
                            (gpui::ease_in_out(delta), current)
                        })
                    })
                });
